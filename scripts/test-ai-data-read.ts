// 📁 scripts/test-ai-data-read.ts
// 测试AI服务是否能正确读取用户修改的简历数据

import { buildResumeContext } from '../lib/ai/aiService';
...
try {
  ...
  // 验证关键信息是否包含
  const checks = [
    { name: '姓名', value: mockResumeData.personalInfo.fullName },
    { name: '职位', value: mockResumeData.personalInfo.title },
    { name: '工作经历', value: mockResumeData.experience[0].company },
    { name: '教育背景', value: mockResumeData.education[0].school },
    { name: '技能', value: mockResumeData.skills[0].name },
    { name: '项目经历', value: mockResumeData.projects[0].name }
  ];
